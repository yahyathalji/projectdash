...
$("#submitOfferBtn").click(async function () {
  ...
  try {
    if (selectedPackages.length > 0) {
      await sendOfferToAPI(
        "http://localhost:5000/api/offers/package",
        selectedPackages
      );
    ...
    .forEach((product) => {
      const row = `
                <tr>
                    ...
                    <td><span class="badge ${getStatusClass(product.Status)}">${
        product.Status
      }</span></td>
    ...
    .forEach((pkg) => {
      const row = `
                <tr>
                    ...
                    <td><span class="badge ${getStatusClass(pkg.Status)}">${
        pkg.Status
      }</span></td>
